...
def main():
    ...
    # Process up to 5 pending videos in a loop if they fail validation constraints
    max_scan_attempts = 5
    for attempt in range(max_scan_attempts):
        # Reload queue and find the next pending video
        queue = []
        if os.path.exists(QUEUE_FILE):
            try:
                with open(QUEUE_FILE, 'r') as f:
        ...
        next_video = pending[0]
        video_id = next_video['id']
        source_url = next_video['source_url']
        title = next_video['title']
        
        logger.info(f"Triggering processing for video ID {video_id} (Attempt {attempt+1}/{max_scan_attempts}): {title}")
        ...
        try:
            ...
            if result.returncode == 12:
                logger.warning(f"Video {video_id} failed validation constraints (too long or horizontal). Marking as SKIPPED_INVALID.")
                update_queue_status(video_id, "SKIPPED_INVALID")
                # Report warning to Telegram
                report_failure(title, "Skipped: Video is too long (>60s) or horizontal.", max(0, 5 - recent_count))
                logger.info("Proceeding to next video in queue...")
